...
class RMSDComplexIsolatedPlotter(BasePlotter):
    ...
    TITLE_FONT_SIZE = 14 # Kept for per-PDB plots 
    ...
    PER_PDB_AXIS_LABEL_FONT_SIZE = 11 # New specific axis label size for per-PDB
    PER_PDB_TICK_LABEL_FONT_SIZE = 10 
    PER_PDB_LEGEND_FONT_SIZE = 9
    ...
    def plot_per_pdb_rmsd_components(self, df, model_type, molecule_type, 
        ...
        if 'RELEASE_DATE' in df_model.columns:
            ...
            df_model = df_model.sort_values('RELEASE_DATE', ascending=True)
        
        pdb_ids = df_model['PDB_ID'].unique()
        if len(pdb_ids) == 0:
            print(f"No PDB IDs found for {model_type}, {molecule_type}, {input_type} per-PDB plot.")
            return [], []
        ...
        paginated_pdb_ids = distribute_pdb_ids(pdb_ids, self.MAX_STRUCTURES_PER_HORIZONTAL_PLOT)

        for page_num, page_pdb_ids_list in enumerate(paginated_pdb_ids, 1):
            page_df = df_model[df_model['PDB_ID'].isin(page_pdb_ids_list)].copy()
            # Preserve order from page_pdb_ids_list
            ...
            ax.invert_yaxis() # PDBs sorted by release date, top to bottom
            ...
            page_info = f" (Page {page_num} of {len(paginated_pdb_ids)})" if len(paginated_pdb_ids) > 1 else ""
            plot_title = f'''{model_type} - {input_type} Ligand
{molecule_type} - Per-PDB RMSD Components{page_info}'''
            fig.suptitle(plot_title, fontsize=self.TITLE_FONT_SIZE, fontweight='bold')
            ...
            if save:
                ...
                filename = f"{model_type.lower()}_{input_type.lower()}_{molecule_type.lower().replace(' ', '_')}_perpdb_rmsd_components{page_suffix}.png"
                save_figure(fig, filename)
